...
class spider_musicChannel:
    
    
    
    def musicChannel(self):
        
        #打开数据库
        db = DB()
        connection = db.open()
        
        url = "https://music.douban.com/tag/"    #请求网址
        ...
        for tag in tags:  
            ...
            helf="https://music.douban.com"                #观察一下豆瓣的网址，基本都是这部分加上标签信息，所以我们要组装网址，用于爬取标签详情页
            url=helf+tag 
            urls.append(url)
            ...
            with connection.cursor() as cursor:
                ...
                sql = 'INSERT INTO musictype(musicTypeInfo)  VALUES ("%s")' % \
                       (type)
                cursor.execute(sql)        
                connection.commit()
        ...
        # 将链接存入文件
        musicchannel = unicode("F:\\Python-Course-Design\\musicchannel.txt","utf-8")    #python处理中文路径
        with open(musicchannel,"w") as file:
            for link in urls:
                    file.write(link+'\n')
